...
  const sharp = clamp(Math.floor(p.t_spiral_sharpness ?? 3), 1, 3); // FIX: Max changed from 8 to 3
  const depth = clamp(p.t_spiral_depth ?? 1.5, 0, 1.5);             // FIX: Max changed from 3.0 to 1.5
  const dual  = p.t_spiral_dual ?? false;                           // FIX: Robust boolean check
...
    t_spiral_bands: 8,
    t_spiral_pitch: 45,
    t_spiral_depth: 1.5, // FIX: Default changed from 2.5 to 1.5 (to match schema)
