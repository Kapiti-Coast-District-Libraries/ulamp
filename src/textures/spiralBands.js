...
function apply(geometry, p) {
  ...
  // FIX: Clamping values now match the schema max
  const bands = clamp(p.t_spiral_bands ?? 8, 2, 64);
  const pitch = clamp(p.t_spiral_pitch ?? 30, 5, 60);
  const sharp = clamp(Math.floor(p.t_spiral_sharpness ?? 3), 1, 3); 
  const depth = clamp(p.t_spiral_depth ?? 1.5, 0, 1.5);             
  const dual  = p.t_spiral_dual ?? false;                           
  const fadeMM = clamp(p.t_spiral_fade_bottom_mm ?? 5, 5, 40);
...
export default {
  ...
  defaults: {
    ...
    t_spiral_dual: false,
    t_spiral_fade_bottom_mm: 5,
  },
  schema: [
    // --- BASIC CONTROLS ---
    { key: "t_spiral_bands",         label: "Strand Count",      type: "range", min: 2,  max: 64, step: 1 },
    { key: "t_spiral_depth",         label: "Texture Depth",     type: "range", min: 0,  max: 1.5, step: 0.05 },
    { key: "t_spiral_dual",          label: "Cross Pattern",     type: "checkbox" },

    // --- ADVANCED CONTROLS ---
    { key: "t_spiral_pitch",         label: "Pitch (Steepness)", type: "range", min: 5,  max: 60, step: 0.5, advanced: true },
    { key: "t_spiral_sharpness",     label: "Ridge Sharpness",   type: "range", min: 1,  max: 3,  step: 0.5, advanced: true },
    { key: "t_spiral_fade_bottom_mm",label: "Fade Bottom mm",    type: "range", min: 5,  max: 40, step: 1,   advanced: true },
  ],
  headroom: (p) => clamp(p.t_spiral_depth ?? 1.5, 0, 1.5),
  apply,
};
